...
export class PostProcessing {
  /**
   * PORT: `options` is a local addition. The original always ran the full
   * pipeline; the card reveal needs to drop the two auxiliary passes and the
   * bloom on low-end devices, so each is now optional. Defaults reproduce the
   * original behaviour exactly.
   *
   * @param {object} [options]
   * @param {boolean} [options.depth]      run the opaque depth prepass (soft particles)
   * @param {boolean} [options.distortion] run the screen-space refraction pass
   * @param {boolean} [options.bloom]      include UnrealBloomPass in the chain
   */
  constructor(renderer, scene, camera, options = {}) {
    ...
    const { depth = true, distortion = true, bloom = true } = options;
    this.useDepth = depth;
    this.useDistortion = distortion;
    this.useBloom = bloom;
    ...
    /* ---- auxiliary buffers ---- */
    // PORT: both buffers are now allocated only when their pass runs.
    ...
    frame.uSceneDepth.value = this.depthTarget?.texture ?? null;
  ...
  render() {
    if (this.useDepth) this._renderDepth();
    if (this.useDistortion) this._renderDistortion();
  ...
  setSize(width, height, pixelRatio) {
    ...
    this.depthTarget?.setSize(halfW, halfH);
    this.distortionTarget?.setSize(halfW, halfH);
