...
const BOLT_ORIGIN = new Vector3(-4.2, 4.4, -1.2);
const BOLT_SIDE = new Vector3(0, 0, 1);
const IMPACT = new Vector3(0, 1.1, 0);
...
export class BattleFxScene {
  constructor(canvas, options = {}) {
    const { quality = 'high', element = 'electric', characterImage = null } = options;
  ...
  _buildBolt() {
    ...
    // Halo underneath, hot core on top. Drawing the glow as real ribbon keeps it
    // attached to every kink instead of smearing off it the way bloom alone would.
    this.boltMaterials = [
      createLightningMaterial(BoltPass.GLOW),
      createLightningMaterial(BoltPass.CORE)
    ];

    this.boltMeshes = this.boltMaterials.map((material, index) => {
  ...
  _buildAura() {
    ...
    this.auraMesh = new Mesh(new IcosahedronGeometry(1, this.quality.tubeSides > 16 ? 4 : 2),
      this.auraMaterial);
    this.auraMesh.position.copy(CHAR_CENTER);
    // Hugging the body: a roomy shell around a flat sprite reads as a bubble.
    this.auraMesh.scale.set(AURA.shellRadius * 0.46, AURA.shellRadius * 0.86, AURA.shellRadius * 0.46);
    this.auraMesh.layers.set(LAYER.VFX);
    this.auraMesh.renderOrder = 10;
  ...
  shockwave() {
    ...
    _pos.set(TARGET.x, 0.5, TARGET.z);
    ...
    this.bursts.spawn(BurstMode.AIR, new Vector3(0, 0.35, 0), {
      radius: SHOCKWAVE.burstRadius,
      endRadius: SHOCKWAVE.burstEndRadius,
      life: SHOCKWAVE.burstLife,
      intensity: 1.1,
      opacity: 0.7,
      squash: 0.35,
      colorA: getColor(p.core),
      colorB: getColor(p.inner),
      colorC: getColor(p.outer)
    });
  ...
  bolt() {
    ...
    this._bolt.t = 0;
    this._bolt.seed = Math.random() * 100;
    this._boltState.seed = this._bolt.seed;
    this._boltState.progress = 0;
    this._boltState.fade = 1;
    this.boltGeometry.instanceCount = BOLT.strands;
    for (const mesh of this.boltMeshes) mesh.visible = true;
  ...
  stopAll() {
    ...
    for (const mesh of this.boltMeshes) mesh.visible = false;
  ...
  _advanceBolt(dt) {
    ...
    if (t < BOLT.travel) {
      this._boltState.progress = Easing.outCubic(saturate(t / BOLT.travel));
      this._boltState.fade = 1;
    } else if (t < BOLT.travel + BOLT.hold) {
      this._boltState.progress = 1;
      this._boltState.fade = 1;
    } else {
      this._boltState.progress = 1;
      this._boltState.fade = 1 - saturate((t - BOLT.travel - BOLT.hold) / BOLT.fade);
    }
    ...
    if (alive) {
      ...
      if (count > 0) {
        ...
        for (let b = 0; b < SPARK_BATCHES; b++) {
          const u = Math.random() * this._boltState.progress;
          _pos.lerpVectors(BOLT_ORIGIN, IMPACT, u);
  ...
  _advanceAura(dt) {
    ...
    this.auraMesh.visible = true;
    this._syncAura(this._auraAmount);
  ...
  _syncBolt() {
    for (const material of this.boltMaterials) material.userData.sync(this._boltState);
  }
  ...
  _syncAura(amount) {
    ...
    this.auraMesh.updateMatrixWorld();
    this._auraState.normalMatrix.getNormalMatrix(this.auraMesh.matrixWorld);
