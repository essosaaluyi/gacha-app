...
/** Diameter of the corona disc, in world units. */
const CORONA_SIZE = 5.2;
...
export class BattleFxScene {
  ...
  _buildBolt() {
    // One quad, billboarded at the character's centre. The twelve ribbon meshes
    // this replaces were drawing a strike; a corona is a field, and a field is
    // cheaper and more controllable as a shader over a disc than as geometry.
  ...
  bolt() {
    // No seed re-roll, no direction fan. The field is continuous; firing only
    // opens an envelope on it, which is what keeps the motion feeling like one
    // living thing rather than a series of separate strikes.
  ...
  _advanceBolt(dt) {
    ...
    // Smooth in, long still hold, smooth out. The field itself never stops
    // evolving; only its visibility is enveloped, so nothing snaps.
    let amount = 1;
    if (t < BOLT.travel) {
    ...
    // A thin bleed of sparks off the corona's mid radius, not off a strike
    // path. Kept low: the brief was fluid and alive, not chaotic.
    if (t < BOLT.travel + BOLT.hold) {
      ...
      if (count > 0) {
        ...
        for (let b = 0; b < SPARK_BATCHES; b++) {
          ...
          const radius = CORONA_SIZE * (0.15 + Math.random() * 0.3);
          _pos.set(
            IMPACT.x + Math.cos(theta) * radius,
            IMPACT.y + Math.sin(theta) * radius,
            IMPACT.z
          );
          _dir.set(Math.cos(theta), Math.sin(theta), 0);
