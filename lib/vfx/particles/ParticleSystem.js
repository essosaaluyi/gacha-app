...
export const ParticleShape = Object.freeze({
  ...
  // PORT: local addition. A four-point star with a hot core, for UI glitter.
  // SOFT reads as a blurred dot at small sizes, which looks cheap on a card.
  SPARKLE: 6
});
  ...
  constructor({
    ...
    this.material = new ShaderMaterial({
      ...
      uniforms: sharedUniforms({
        ...
        // PORT: only read by SHAPE == SPARKLE.
        uSparkNeedle: { value: 0.05 },
        // PORT: only read by SHAPE == RING. See the shape function.
        uRingThin: { value: 0 },
...
const PARTICLE_FRAGMENT = /* glsl */ `
  ...
  uniform float uSparkNeedle; // PORT: SPARKLE needle thinness; lower is sharper
  uniform float uRingThin;    // PORT: RING stroke stays constant as it expands
  uniform float uEndSize;     // PORT: RING needs the same growth the vertex used
  ...
  float shapeMask(vec2 uv) {
    ...
    #elif SHAPE == 6                     // SPARKLE  (PORT: local addition)
      // Reciprocal falloff along each axis, not a gaussian: 1/(x+k) keeps a
      // needle thin far from centre where a smoothstep would have gone soft.
      // That thinness is the whole difference between a sparkle and a dot.
      float ax = abs(c.x);
      float ay = abs(c.y);
      float taper = max(0.0, 1.0 - d);   // needles have to die before the quad edge

      // Quadratic length falloff, not linear: a linear taper eats the arm
      // before it gets anywhere and the star collapses back into a dot.
      float hor = uSparkNeedle / (ay + uSparkNeedle) * max(0.0, 1.0 - ax * ax);
      float ver = uSparkNeedle / (ax + uSparkNeedle) * max(0.0, 1.0 - ay * ay);

      // A second, dimmer pair rotated 45°: four points read as a cross, eight
      // read as something catching the light.
      vec2 r = vec2(c.x + c.y, c.y - c.x) * 0.70710678;
      float rx = abs(r.x);
      float ry = abs(r.y);
      float hor2 = uSparkNeedle / (ry + uSparkNeedle) * max(0.0, 1.0 - rx * rx);
      float ver2 = uSparkNeedle / (rx + uSparkNeedle) * max(0.0, 1.0 - ry * ry);

      float spikes = (hor + ver) + (hor2 + ver2) * 0.3;
      // Tight core. A wide one washes over the inner third of the arms and
      // takes the cross with it.
      float core = exp(-d * 16.0);
      return clamp(spikes * taper * 0.9 + core, 0.0, 1.0);

    #else                                // RING
      // PORT: the band is a fraction of the quad, so a ring that grows 14x
      // draws its stroke 14x thicker and ends as a solid donut rather than a
      // wave front. uRingThin divides the band by the growth the vertex shader
      // applied, holding the stroke at a constant width in pixels. It defaults
      // to 0, which is the original behaviour exactly.
      float growth = mix(1.0, mix(1.0, uEndSize, vT), clamp(uRingThin, 0.0, 1.0));
      return smoothstep(0.14 / max(growth, 1e-3), 0.0, abs(d - 0.82));
    #endif
  }
