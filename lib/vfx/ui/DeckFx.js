...
/**
 * Extra glitter on top of the proportional scale. Sizing it to the deck alone
 * kept the field honest but left it reading as dust on a disc this size; the
 * shimmer is meant to catch the eye while the player decides to press again.
 */
const GLITTER_BOOST = 1.35;
/**
...
export class DeckFx {
  ...
  armDraw(deckEl, outcome, options = {}) {
    ...
    // `force` defeats the withholding for testing only. It cannot conjure a
    // tell out of nothing — a hand that earned none still gets none, so this
    // can never show the aura on an ordinary draw.
    const show = rules.auraChance > 0 && (options.force || Math.random() < rules.auraChance);
    ...
    this.layer.startGlitter(rect, this.glitterTone, shape, this._deckScale(rect) * GLITTER_BOOST);
    return true;
  }
  ...
  _glowThickness(rect, tell) {
    return tell.thickness * this._deckScale(rect);
  }
   ...
   */
  drawCard(deckEl, landing, outcome, durationSeconds = 0.45) {
    const rules = rulesFor(outcome);
