 ...
 *
 * Each entry is one rung: a colour, how hard it glows, and how much spectacle
 * the draw itself gets. The rung is an *expectancy*, not an outcome — white
 * appears constantly and means almost nothing, gold is rare and never lies.
 * What decides which rung a draw gets lives in lib/battle-pixi/presentation/
 * drawTell.ts, next to the odds it is derived from; this file only says what
 * each rung looks like.
 *
 * Escalation runs on two axes so the low rungs stay cheap. White and blue get
 * the glow and the glitter only; green and above add the arc discharge, which
 * is what makes the jump from blue to green feel like a different kind of
 * event rather than a brighter one.
 */
