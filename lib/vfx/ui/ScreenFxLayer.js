...
export class ScreenFxLayer {
  constructor(canvas, options = {}) {
    ...
    /* ---- draw-trail state ---- */
    /** One entry per card in flight; each carries its own emitter. */
    this._trails = [];

    /* ---- glitter state ---- */
    this._glitter = { active: false, rect: null, tone: 'white', shape: 'rect', scale: 1, amount: 0 };
   ...
   */
  showGlow(rect, tellName, radius = 14, thickness = null) {
    this._glow.rect = rect;
    this._glow.radius = radius;
    this._glow.tell = tellFor(tellName);
    // The tells carry a thickness in pixels, which only reads correctly at the
    // size they were tuned at. A caller that knows the element's real size can
    // override it; see DeckFx.armDraw.
    this._glow.thickness = thickness;
    this._glow.target = 1;
   ...
   * @param {'rect'|'disc'} [shape] the face to scatter across. A disc deck in a
   *   rectangular rect would take a fifth of its glitter outside the art.
   */
  /**
   * @param {number} [prime] fraction of the settled field to scatter at once on
   *   the first frame. 1 lands straight on steady state; 0 ramps from nothing.
   */
  startGlitter(rect, tone = 'white', shape = 'rect', scale = 1, prime = 0) {
    ...
    this._glitter.shape = shape;
    // Sizes and emission rate are in pixels and pixels-worth-of-face per
    // second, so both have to grow with the face they are scattered over or a
    // large deck reads as a few specks of dust.
    this._glitter.scale = scale;
    this._glitter.amount = 1;

    // Seed the field so the cue is legible on the frame the button is pressed,
    // rather than a second and a half later once the rate emitter has caught
    // up. Everything after this is the steady shimmer.
    //
    // `prime` is a fraction of the field's own steady state — rate x life is
    // how many glitters are alive at once once it settles — rather than a
    // count. A count would have to be retuned every time the rate, the
    // lifetime or the deck's size changed; a fraction stays meaningful.
    if (prime > 0) {
      this._emitGlitter(Math.round(GLITTER.rate * scale * GLITTER.life * prime));
    }
  }
   ...
   */
  drawTrail(from, to, tellName, duration = TIMING.draw, options = {}) {
    ...
    // Each trail owns its emitter. A shared one would hand the fractional
    // remainder of one card's emission to the next, which is how three trails
    // with the same rate end up with visibly different densities.
    if (replace) this._trails.length = 0;
    this._trails.push({
      from, to, tell, duration, delay, life, spread, sizeScale, rateScale,
      t: 0,
      emitter: new RateEmitter(0)
    });
  }
  ...
  _advanceGlow(dt) {
    ...
    s.radius = g.radius;
    s.thickness = g.thickness ?? g.tell.thickness;
  ...
  _advanceGlitter(dt) {
    ...
    const count = this._glitterEmitter.tick(dt, GLITTER.rate * (g.scale ?? 1));
    if (count > 0) this._emitGlitter(count);
  }

  /**
   * Scatter `count` glitters across the current face.
   *
   * Split out of the update loop so the same placement can be used to seed the
   * field on the first frame. A rate emitter alone takes a second or two to
   * establish a readable field, which is fine for an accent on top of
   * something else but not for a cue that has to land the moment the button is
   * pressed.
   */
  _emitGlitter(count) {
    const g = this._glitter;
    if (!g.rect || count <= 0) return;
    ...
    const disc = g.shape === 'disc';
    const cx = left + width / 2;
    const cy = top + height / 2;

    for (let i = 0; i < count; i++) {
      if (disc) {
        // sqrt of a uniform, not the uniform itself: r = U would crowd the
        // middle, because a ring's area grows with its radius.
        const r = Math.sqrt(Math.random());
        const a = Math.random() * Math.PI * 2;
        _pos.set(cx + Math.cos(a) * r * width / 2, cy + Math.sin(a) * r * height / 2, 0);
      } else {
      ...
      this.glitter.emit(1, {
        ...
        size: (GLITTER.sizeMin + (GLITTER.sizeMax - GLITTER.sizeMin) * Math.pow(Math.random(), GLITTER.sizeBias)) * scale,
        sizeVariance: 0,
        life: GLITTER.life,
  ...
  _advanceTrail(dt) {
    ...
    for (let i = this._trails.length - 1; i >= 0; i--) {
      ...
      // A card that has not left yet still owns a trail — it just has not
      // started drawing it. Staggered departures are what make three cards
      // read as three cards rather than as one wide smear.
      if (t.t < t.delay) continue;

      const u = saturate((t.t - t.delay) / t.duration);

      // Follow the card's eased path, and throw the particles the way it goes.
      ...
      const rate = t.tell.trailRate * t.rateScale * (1 - u * 0.55);
      const count = t.emitter.tick(dt, rate);

      if (count > 0) {
        _tint.copy(getColor(t.tell.color));
        _pos.set(x, y, 0);
        // Backwards along the path: a trail is what the card leaves behind, so
        // throwing the particles forward would put them ahead of it.
        _dir.set(-dx / length, -dy / length, 0);

        // Slow and tight. A trail is particles the card *left*, so they should
        // hang near the line it took; anything with real speed or a wide cone
        // scatters off the path and reads as a cloud the card flew through.
        this.trail.emit(count, {
          ...
          speedVariance: 0.7,
          spread: 0.4,
          size: t.tell.trailSize * t.sizeScale,
          sizeVariance: 0.6,
          life: t.life,
          lifeVariance: 0.45,
          tint: _tint,
      ...
      if (u >= 1) this._trails.splice(i, 1);
    }
  }
