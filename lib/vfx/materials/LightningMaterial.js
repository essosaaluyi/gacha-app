...
const BOLT_VERTEX = /* glsl */ `
  ...
  void main() {
    ...
    // PORT: an orthographic camera has no eye point to aim at. Under one,
    // cameraPosition is just wherever the camera object happens to sit, and for
    // a flat scene on the z = 0 plane that vector lies *in* that plane — so the
    // binormal comes out along z and the ribbon expands into the screen with
    // zero visible width. Every view ray is parallel instead, along the camera's
    // own axis, which is row 2 of the view matrix. Perspective keeps the
    // per-vertex vector, which is what makes a bolt hold its thickness when it
    // passes close to the eye. isOrthographic comes from three's own vertex
    // prefix, which declares and sets it for every ShaderMaterial.
    vec3 toCamera = isOrthographic
      ? vec3(viewMatrix[0][2], viewMatrix[1][2], viewMatrix[2][2])
      : normalize(cameraPosition - here);
